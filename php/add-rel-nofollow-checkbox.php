...
/**
 * Add a 'Add rel="nofollow noopener noreferrer" to link' checkbox to the WordPress link editor
 * Customized
 *
	...
	<script>
		...
		if ( typeof tinymce !== 'undefined' && typeof _ !== 'undefined' && typeof wpLink !== 'undefined' ) {
			...
			if ( tinymce.$('#link-options').length ) {
				...
				tinymce.$('#link-options').append(<?php echo json_encode( '<div class="link-nofollow"><label><span></span><input type="checkbox" id="wp-link-nofollow" /> Add rel="nofollow noopener noreferrer" to link</label></div>' ); ?>);
            ...
            attrs.rel = wpLink.addRelNofollow.prop( 'checked' ) ? 'nofollow noopener noreferrer' : null;
						return attrs;
					},
					/**
...
/**
 * Remove default rel="noopener noreferrer" with target='-blank' in TinyMCE links
 *
 * @param  array $init
 * @return array
 */
